...
@router.get("/rut/{rut}", response_model=PacienteOut)
async def read_paciente_by_rut(rut: str, session: AsyncSession = Depends(get_session)):
    paciente = await crud.get_paciente_by_rut(session, rut)
    if paciente is None:
        raise HTTPException(status_code=404, detail="Paciente no encontrado")
    return paciente
